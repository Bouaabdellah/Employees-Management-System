...
function EditWorkInfo({ employee }: { employee: employee }) {
  ...
  const validateData = (): boolean => {
    ...
    const thereIsChange =
      employee.salary !== workInfo.salary ||
      employee.branch_id !== workInfo.branchID ||
      employee.role_id !== workInfo.roleID ||
      employee.super_id !== workInfo.mgrID ||
      employee.start_day !== workInfo.startDate ||
      employee.is_manager !== workInfo.is_manager;
    return validateSalary && validateStartDay && thereIsChange;
  };
  const sendData = async () => {
    try {
      if (validateData()) {
        axios.put(`${host}/employees/update`, {
          id: workInfo.id,
  ...
  const deleteEmployee = async () => {
    try {
      axios.delete(`${host}/employees/delete`, {
        data: {
          empID: workInfo.id,
        },
      });
  ...
  return (
    <div className="mt-8">
      <div className="flex justify-between mb-8">
        ...
        <button
          className="capitalize py-2 px-4 text-white rounded-md
        bg-red-600 duration-300 hover:bg-red-700"
          onClick={(e) => deleteEmployee()}
        >
          delete employee
        </button>
      </div>
        ...
        <div className="pl-2">
          <table>
            ...
            <tbody>
              <tr>
                ...
                <td className="py-2">
                  <select
                    ...
                    onChange={(e) =>
                      setWorkInfo({ ...workInfo, branchID: +e.target.value })
                    }
                  >
                    {branches.map((ele: branch) => {
                      return (
                        <option
                          value={ele.branch_id}
                          key={ele.branch_id}
                          selected={ele.branch_id === employee.branch_id}
                        >
                          {ele.branch_name}
                        </option>
                      );
              ...
              <tr>
                ...
                <td className="py-2">
                  <select
                    ...
                    className="bg-gray-200 py-2 px-4 rounded-md"
                    onChange={(e) =>
                      setWorkInfo({ ...workInfo, roleID: +e.target.value })
                    }
                  >
                    {roles.map((ele: role) => {
                      return (
                        <option
                          value={ele.role_id}
                          key={ele.role_id}
                          selected={ele.role_id === employee.role_id}
                        >
                          {ele.role_name}
                        </option>
                      );
              ...
              <tr>
                ...
                <td className="py-2">
                  <select
                    ...
                    className="bg-gray-200 py-2 px-4 rounded-md"
                    onChange={(e) =>
                      setWorkInfo({ ...workInfo, mgrID: +e.target.value })
                    }
                  >
                    {managers.map((ele: { super_id: number }) => {
                      return (
                        <option
                          value={ele.super_id}
                          key={ele.super_id}
                          selected={ele.super_id === employee.super_id}
                        >
                          {ele.super_id}
                        </option>
                      );
                    })}
                    <option value="0" selected={!employee.super_id}>
                      none
                    </option>
                  </select>
                </td>
                ...
                <td className="py-2">
                  <label
                    ...
                    className={`text-[18px] mr-4 ${
                      !validate.startDate && 'text-red-600'
                    }`}
                  >
                    start date
                  </label>
              ...
              <tr>
                ...
                <td className="py-2">
                  <input
                    ...
                    onChange={(e) =>
                      setWorkInfo({ ...workInfo, salary: +e.target.value })
                    }
              ...
              <tr className="py-2">
                <td>
                  <label htmlFor="is_manager" className={`text-[18px] mr-4`}>
                    is manager
                  </label>
                </td>
                <td>
